...
public enum ResponseEnum {
	 ...
	 * 拒绝该用户
	 */
	USER_REJECTED(401, "The user was rejected"),
	 ...
	 * 数据异常
	 */
	DATA_ERROR(-4, "数据异常，请刷新后重新操作"),

	/**
	 * 一些需要登录的接口，而实际上因为前端无法知道token是否已过期，导致token已失效时，
	 * 应该返回一个状态码，告诉前端token已经失效了，及时清理
	 */
	CLEAN_TOKEN(401, "clean token"),

	/**
	 * 刷新token已过期
	 */
	REFRESH_TOKEN_EXIST(405, "Refresh token has expired"),

	/**
	 * Token已失效
	 */
	TOKEN_HAS_EXPIRED(405, "Token has expired"),

	/**
	 * 数据不完整
	 */
	DATA_INCOMPLETE(-5, "数据不完整"),
